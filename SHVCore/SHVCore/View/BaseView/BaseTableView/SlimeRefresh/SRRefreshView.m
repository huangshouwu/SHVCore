...
@property (nonatomic, assign) CGFloat iPhoneXOffSet; //在iPhone X界面tableview如果从顶部开始,则为了不让刘海挡住刷新视图,将刷新视图像下偏移25个点.默认0
@end
...
- (instancetype)initWithHeight:(CGFloat)height
{
    ...
    if (self = [super initWithFrame:frame]) {
        ...
        _imgTitleButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 70.0f, height)];
...
- (void)setLoading:(BOOL)loading
{
    ...
    if (_loading) {
        ...
        aniamtion.values = [NSArray arrayWithObjects:
                            [NSValue valueWithCATransform3D:CATransform3DRotate(CATransform3DMakeScale(0.1, 0.1, 0.1), -M_PI, 0, 0, 1)],
        ...
        __weak typeof(_imgTitleButton) block_imgTitleButton = _imgTitleButton;
        __weak typeof(_scrollView) block_scrollView = _scrollView;
...
{
    if (_loading) {
        [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(callStartRefresh) object:nil];
        [self callStartRefresh];
        return;
    }
...
- (void)scrollViewDidScroll
{
    ...
    /* 拖拽距离大于(dragingHeight + _upInset),即处于拉伸的状态下 */
    if (p.y <= - _dragingHeight - _upInset - self.iPhoneXOffSet) {
        rect.origin.y = p.y + _upInset + self.iPhoneXOffSet;
        rect.size.height = -p.y;
        rect.size.height = ceilf(rect.size.height);
        self.frame = rect;

        _refleshView.hidden = _slime.hidden = self.loading;
        ...
        /* 在没有拉断时,处理_refleshView的放大缩小 */
        if (!_broken) {
            float l = -(p.y + _dragingHeight + _upInset + self.iPhoneXOffSet);
...
- (void)scrollViewDidEndDraging
{
    ...
    /* 让_scrollView停在_upInset + _dragingHeight这个位置(此时_activityIndicatorView正在animating) */
    if (_scrollView && self.loading) {
        ...
        [UIView transitionWithView:_scrollView
                        ...
                        animations:^{
                            UIEdgeInsets inset = block_scrollView.contentInset;
                            inset.top = _upInset + _dragingHeight + self.iPhoneXOffSet;
                            block_scrollView.contentInset = inset;
                        } completion:^(BOOL finished) {
...
{
    _slime.toPoint = _slime.startPoint;
    __weak typeof(_slime) block_slime = _slime;
    __weak typeof(_activityIndicatorView) block_activityIndicatorView = _activityIndicatorView;
    __weak typeof(self) block_self = self;
...
#pragma mark - 调用delegate和block
- (void)callStartRefresh
{
    if (_delegate && [_delegate respondsToSelector:@selector(slimeRefreshStartRefresh:)]) {
        [_delegate performSelector:@selector(slimeRefreshStartRefresh:) withObject:self];
    }
