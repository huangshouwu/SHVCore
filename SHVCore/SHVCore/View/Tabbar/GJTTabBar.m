...
- (id)initWithItems:(NSArray*)items frame:(CGRect)frame{
    ...
    if (self){
        ...
        for (GJTTabBarItem* item in items) {
            NSUInteger index = [items indexOfObject:item];
            rect.origin.x = index*_buttonWidth;
            GJTTabBarButton* button = [[GJTTabBarButton alloc] initWithItem:item frame:rect];
...
- (void)setSelectedItem:(NSUInteger)index{
    ...
    if (index<self.items.count){
        GJTTabBarItem* item = [self.items objectAtIndex:index];
        for (UIView* view in self.subviews) {
            if ([view isMemberOfClass:[GJTTabBarButton class]]){
                GJTTabBarButton* button = (GJTTabBarButton*)view;
                if (item==button.item){
...
- (NSUInteger)indexOfObject:(GJTTabBarButton*)object{
    if (object){
        for (GJTTabBarItem* item in self.items) {
            if (item == object.item){
...
- (GJTTabBarButton*)buttonAtIndex:(NSInteger)index{
...
- (void)setBadgeValue:(NSString*)badgeValue atIndex:(NSInteger)index{
    GJTTabBarButton* button = [self buttonAtIndex:index];
    if ([button isKindOfClass:[GJTTabBarButton class]]) {
...
+ (id)itemWithTitle:(NSString *)title image:(UIImage *)image hightLightImage:(UIImage*)hightLightedImage color:(UIColor*)color hightLightedColor:(UIColor*)hightLightColor tag:(NSUInteger)tag;{
    GJTTabBarItem* item = [[self alloc] init];
    item.title = title;
    item.image = image;
