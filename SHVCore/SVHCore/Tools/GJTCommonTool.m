...
UIViewController* GJTFindTopModelViewController(UIViewController* vc){
...
UIViewController* GJTVisibalController(){
    ...
    if ([appRootViewController isKindOfClass:[UITabBarController class]]) {
        ...
        if (tableBarVC.presentedViewController) {
            visibalVC = GJTFindTopModelViewController(tableBarVC);
        }else {
            ...
            if (selectedNav.presentedViewController) {
                visibalVC = GJTFindTopModelViewController(selectedNav);
            }else {
        ...
        if (appRootViewController.presentedViewController) {
            visibalVC = GJTFindTopModelViewController(appRootViewController);
        }else {
...
NSString* GJTRandomSingleChineseString(){
...
NSString* GJTRandomChineseString(NSUInteger length){
    if (length >0) {
        ...
        for (NSInteger i =0 ; i<length; i++) {
            [string appendString:GJTRandomSingleChineseString()];
        }
        return string;
    }else {
        return GJTRandomSingleChineseString();
    }
}
...
NSString* GJTCallPhoneInView(NSString* phone,UIView* inView){
    ...
    if([strDeviceType  isEqualToString:@"iPod touch"] || [strDeviceType  isEqualToString:@"iPad"] || [strDeviceType  isEqualToString:@"iPhone Simulator"]){
        ...
        if ([[UIApplication sharedApplication] canOpenURL:URL]) {
            ...
            return [NSString stringWithFormat:@"无法拨打电话号码:%@",phone];
        }
    }
    return nil;
}
