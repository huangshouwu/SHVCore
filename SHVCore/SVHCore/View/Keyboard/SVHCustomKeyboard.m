...
#import "SVHCustomKeyboard.h"
#import "SVHCommonTool.h"
#import "SVHBaseMacros.h"
#import "SVHUIViewAdditions.h"

static CGFloat        const SVHCustomKeyboardDeleteButtonWidth            = 80.0f;
static NSInteger      const SVHCustomKeyboardNumberButtonCountEveryRow    = 3;
static NSInteger      const SVHCustomKeyboardNumberButtonCountEveryCollom = 4;
static UIEdgeInsets   const SVHCustomKeyboardNumberContentInsets          = {0.5f,0.0f,0.0f,0.0f};
static CGFloat        const SVHCustomKeyboardNumberButtonHeight           = 50.0f;

@interface SVHCustomKeyboard ()<UIGestureRecognizerDelegate>{
    NSArray                *_numerPadArray;/* 数字区域的值 */
    NSMutableArray         *_numerPadButtonArray;/* 数字区域的按钮 */
    SVHCustomKeyboardType _keyboardType;
    NSTimer                *_deleteTimer;
}

@property (nonatomic,strong)SVHDeleteButton* deleteButton;
@property (nonatomic,strong)SVHBaseButton* registerFirstResponderButton;
...
@implementation SVHCustomKeyboard

#pragma mark Public-Method
+ (CGFloat )keyBoardHeight{
    return SVHCustomKeyboardNumberContentInsets.top + SVHCustomKeyboardNumberContentInsets.bottom +  (1.0 / [UIScreen mainScreen].scale)*(SVHCustomKeyboardNumberButtonCountEveryCollom-1) + SVHCustomKeyboardNumberButtonCountEveryCollom*SVHCustomKeyboardNumberButtonHeight + SVHBottomSafeMargin;
}

+ (instancetype)keyBoardWithType:(SVHCustomKeyboardType)type{
    return [[SVHCustomKeyboard alloc] initWithFrame:CGRectMake(0, 0, CGRectGetWidth([UIScreen mainScreen].bounds), [SVHCustomKeyboard keyBoardHeight]) type:type];
}

- (instancetype)initWithFrame:(CGRect)frame type:(SVHCustomKeyboardType)type{
    self = [super initWithFrame:frame];
    if (self) {
        _keyboardType = type;
        //背景色为分割线
        self.backgroundColor = SVHColorForHex(@"#cccccc");
...
- (void)layoutSubviews{
    ...
    CGRect rowOneLastButtonFrame = [self numberPadButtonFrameAtIndex:SVHCustomKeyboardNumberButtonCountEveryRow-1];
    CGFloat lineHeight = 1.0f/[[UIScreen mainScreen] scale];
    self.deleteButton.frame = CGRectMake(CGRectGetMaxX(rowOneLastButtonFrame)+lineHeight, SVHCustomKeyboardNumberContentInsets.top, SVHCustomKeyboardDeleteButtonWidth, 2*SVHCustomKeyboardNumberButtonHeight);
    self.sureButton.frame = CGRectMake(CGRectGetMinX(self.deleteButton.frame) , CGRectGetMaxY(self.deleteButton.frame), CGRectGetWidth(self.deleteButton.frame), CGRectGetHeight(self.frame) - CGRectGetMaxY(self.deleteButton.frame) - SVHCustomKeyboardNumberContentInsets.bottom - SVHBottomSafeMargin);
    
    if (SVHIsiPhoneX) {
        self.bottomSafeView.frame = CGRectMake(0, CGRectGetMaxY(self.sureButton.frame), CGRectGetWidth(self.bounds), SVHBottomSafeMargin);
...
- (void)layoutNumberPadButton{
    for (NSInteger i = 0;i<[_numerPadButtonArray count];i++) {
        SVHBaseButton* numberButton = _numerPadButtonArray[i];
...
- (CGRect)numberPadButtonFrameAtIndex:(NSInteger)index{
    ...
    CGFloat lineHeight = 1.0f/[[UIScreen mainScreen] scale];
    CGPoint numberButtonPosition = CGPointMake((index%SVHCustomKeyboardNumberButtonCountEveryRow)*(numberButtonSize.width + lineHeight) + SVHCustomKeyboardNumberContentInsets.left, (index/SVHCustomKeyboardNumberButtonCountEveryRow)*(numberButtonSize.height + lineHeight) + SVHCustomKeyboardNumberContentInsets.top);
    return CGRectMake(numberButtonPosition.x, numberButtonPosition.y, numberButtonSize.width, numberButtonSize.height);
}

#pragma mark SubViews
- (SVHDeleteButton*)deleteButton{
    if (!_deleteButton) {
        UIImage *imgNormal      = [self buttonNomalBackgroundImage];
        UIImage *imgHighLighted = [self buttonHightlightBackgroundImage];
        _deleteButton = [[SVHDeleteButton alloc] initWithFrame:CGRectZero];
        [_deleteButton setImage:SVHResourceImageWithName(@"GJTKeyboardDeleteButton") forState:UIControlStateNormal];
        ...
        _deleteButton.touchBlock = ^(SVHDeleteButton* button,SVHDeleteButtonTouchState state){
            switch (state) {
                case SVHDeleteButtonTouchStateBegan:
                    [block_self performSelector:@selector(startDeleteText) withObject:nil afterDelay:0.3];
                    break;
                case SVHDeleteButtonTouchStateCancelled:
                case SVHDeleteButtonTouchStateEnded:
...
- (SVHBaseButton*)registerFirstResponderButton{
    if (!_registerFirstResponderButton) {
        UIImage *imgNormal      = [self buttonNomalBackgroundImage];
        UIImage *imgHighLighted = [self buttonHightlightBackgroundImage];
        _registerFirstResponderButton = [[SVHBaseButton alloc] initWithFrame:CGRectZero];
        [_registerFirstResponderButton setImage:SVHResourceImageWithName(@"GJTKeyboardHideButton") forState:UIControlStateNormal];
...
- (SVHBaseButton*)sureButton{
    if (!_sureButton) {
        _sureButton = [[SVHBaseButton alloc] initWithFrame:CGRectZero];
        [_sureButton setTitle:@"确定" forState:UIControlStateNormal];
        [_sureButton setBackgroundImage:[self buttonImageFromColor:SVHColorForHex(@"#00aaee")] forState:UIControlStateNormal];
        [_sureButton setTitleColor:[UIColor whiteColor] forState:(UIControlStateNormal)];
        [_sureButton setTitleColor:SVHColorForHex(@"#80d5f7") forState:UIControlStateDisabled];
...
- (void)fillNumberPadArrayWithType:(SVHCustomKeyboardType)type{
    ...
    switch (type) {
        case SVHCustomKeyboardTypeNumberPad:
            _numerPadArray = [[NSArray alloc] initWithObjects:@"1",@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@"",@"0", nil];
            break;
        case SVHCustomKeyboardTypeNumAndDotPad:
            _numerPadArray = [[NSArray alloc] initWithObjects:@"1",@"2",@"3",@"4",@"5",@"6",@"7",@"8",@"9",@".",@"0", nil];
            break;
        case SVHCustomKeyboardTypeNumAndXPad:
...
- (void)createNumberPadButtonWithType:(SVHCustomKeyboardType)type{
    ...
    for (NSInteger i = 0 ; i < [_numerPadArray count] ;i ++) {
        ...
        SVHBaseButton* numberButton = [SVHBaseButton buttonWithType:UIButtonTypeCustom];
...
- (void)numberPadButtonTaped:(SVHBaseButton*)sender{
    UITextField* _responderTextField = [self currentResponderTextField];
...
- (void)deleteButtonTaped:(SVHBaseButton*)sender{
...
- (void)registerFirstResponderButtonTaped:(SVHBaseButton*)sender{
    [self playClickSound];
    [self registerFirstResponder];
}

- (void)sureButtonTaped:(SVHBaseButton*)sender{
    [self playClickSound];
    [self registerFirstResponder];
}
...
- (CGSize)numberPadButtonSize{
    CGFloat lineHeight = 1.0f/[[UIScreen mainScreen] scale];
    return  CGSizeMake(((CGRectGetWidth(self.bounds) - SVHCustomKeyboardDeleteButtonWidth) - SVHCustomKeyboardNumberContentInsets.left - SVHCustomKeyboardNumberContentInsets.right - (SVHCustomKeyboardNumberButtonCountEveryRow - 1)*lineHeight)/SVHCustomKeyboardNumberButtonCountEveryRow, SVHCustomKeyboardNumberButtonHeight);
}
...
@implementation SVHDeleteButton

- (void)touchesBegan:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    [super touchesBegan:touches withEvent:event];
    if (self.touchBlock) {
        self.touchBlock(self,SVHDeleteButtonTouchStateBegan);
    }
}
...
- (void)touchesCancelled:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (self.touchBlock) {
        self.touchBlock(self,SVHDeleteButtonTouchStateCancelled);
    }
}
...
- (void)touchesEnded:(NSSet<UITouch *> *)touches withEvent:(UIEvent *)event{
    ...
    if (self.touchBlock) {
        self.touchBlock(self,SVHDeleteButtonTouchStateEnded);
    }
}
