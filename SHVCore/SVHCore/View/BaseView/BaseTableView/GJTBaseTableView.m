...
@implementation GJTBaseTableView
...
- (instancetype)initWithFrame:(CGRect)frame style:(UITableViewStyle)style showDragView:(BOOL)showDragView
{
    if (self = [super initWithFrame:frame]) {
        ...
        _tableView = [[GJTRefreshTableView alloc] initWithFrame:CGRectMake(0, 0, frame.size.width, frame.size.height) style:style];
        _tableView.dataSource = self;
        _tableView.delegate = self;
        _tableView.showRefreshView = showDragView;
        __weak typeof(self) bself = self;
        if (_tableView.showRefreshView) {
            _tableView.startRefreshBlock = ^(GJTRefreshTableView *resreshTableView) {
...
{
    GJTBaseCellItem* object = [self tableView:tableView objectForRowAtIndexPath:indexPath];
    if (!object ||![object isKindOfClass:[GJTBaseCellItem class]]){
//        NOEEDPRINT(@"item 已经不存在了!!");
    ...
    if ([cell isKindOfClass:[GJTBaseCell class]]) {
        [(GJTBaseCell*)cell setItem:object];
    }
    return cell;
}
...
- (void)didSelectObject:(GJTBaseCellItem *)object atIndexPath:(NSIndexPath *)indexPath{
